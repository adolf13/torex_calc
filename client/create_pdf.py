...
def png_to_pdf_with_images_and_text(png_filepath, pdf_filepath, top_text, pagesize=letter):
    ...
    try:
        ...
        new_img = Image.new("RGB", (new_width+300, new_height+300), "white")

        # Paste torex image
        new_img.paste(torex_img, (1400, 0))  # 10px padding

        # Paste the main image
        new_img.paste(img, (torex_img.width, top_text_height+100))  # 20px padding
        ...
        for line in bottom_text:
            draw.text((torex_img.width, y_offset + 100), line, font=font, fill="black")  # Add padding
            y_offset += text_height_per_line
